...
from tensorflow.examples.tutorials.optdigits import  input_data
...
trainwriter=tf.summary.FileWriter('data/optdigits_model'+'/logs/train',sess.graph)

init = tf.global_variables_initializer()
sess.run(init)

for i in range(1000):
    batch_xs, batch_ys = optdigits.train.next_batch(100)
    summary, _ = sess.run([merged, train_step], feed_dict={x: batch_xs, y_: batch_ys})
    trainwriter.add_summary(summary, i)

# model export path
export_path = 'data/optdigits_model'
...
saver.save(sess, '/home/manikanta/tensorflow/optdigits_model')
"""